...
struct FileGridView: View {
    ...
    /// Un geste vertical demande l'affichage (bas) ou le masquage (haut),
    /// quelle que soit la position dans la liste. L'inertie ne décide jamais.
    var onSearchVisibilityRequested: ((Bool) -> Void)?
    ...
    @GestureState private var searchDragDecision: Bool? = nil
        ...
        ScrollViewReader { proxy in
            ...
            .simultaneousGesture(
                DragGesture(minimumDistance: 12, coordinateSpace: .global)
                    .updating($searchDragDecision) { value, decision, _ in
                        guard onSearchVisibilityRequested != nil,
                              decision == nil,
                              abs(value.translation.height) >= 24,
                              abs(value.translation.height) > abs(value.translation.width) * 1.5
                        else { return }
                        // Une seule décision par contact, même si la liste
                        // rebondit ou si le doigt inverse ensuite sa course.
                        // GestureState se réinitialise aussi en cas d'annulation.
                        decision = value.translation.height > 0
                    }
            )
            .onChange(of: searchDragDecision) { _, decision in
                guard let decision else { return }
                onSearchVisibilityRequested?(decision)
            }
